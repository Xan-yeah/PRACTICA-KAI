...
// interface2.xaml.cs - ТЕРМИНАЛ №2 (оплата парковки).
//Программу разработали: Гарафутдинов А.Р.; Зайнабиддинов И.К.
//---------
//Детали по условностям:
// Формула расчёта:
//   stoyankaMinut = (тек.время - время заезда) в минутах
//   если stoyankaMinut <= M               -> бесплатно (0 руб)
//   иначе platnyeMinuty = stoyankaMinut - M
//         chasyKOplate  = ЦЕЛОЕ ОКРУГЛЕНИЕ ВВЕРХ (platnyeMinuty / 60)
//         summaKOplate  = chasyKOplate * CenaZaChas
...
namespace AvtoParkingKAI
{
    public partial class interface2 : KioskWindow
    {
        ...
        private DispatcherTimer bankTaymer;    // крутит ProgressBar "банка"
        ...
        public interface2()
        {
            ...
            // "банк думает": 50 мс * 60 шагов = 3 секунды соединения
        ...
        private void UsbKarta_KartaVstavlena()
        {
            ...
            // режим ожидания диспетчера: принимаем ТОЛЬКО его служебную флешку
            if (zhdemDispKlyuch)
            {
                PrinyatKlyuchDispetcheraEsliEst();
                return; // остальные флешки в этом режиме игнорируем
            }

            // льгота активна и вставили карту клиента - гасим оверлей и оформляем
            if (lgotaAktivna)
            {
