...
//   2) AES-256 шифрование файла pass.dat на флешке: ключ и вектор IV
//      выводятся из секретной фразы через SHA-256/MD5, наружу торчит base64
//   3) эталонный хэш ключа диспетчера (файл disp_key.dat, льготный проезд)
// ============================================================================
...
namespace AvtoParkingKAI.Services
{
    ...
    public static class BezopasnostService
    {
        ...
        {
            // защита от дурака: битая/подделанная карта не должна ронять
            // терминал, поэтому весь блок обёрнут в try-catch
            try
            {
                using (Aes aes = Aes.Create())
                {
