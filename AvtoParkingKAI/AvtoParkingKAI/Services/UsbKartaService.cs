...
// v1: опрашивал DriveInfo.GetDrives() таймером каждые 2 секунды:
//   while (true) { foreach (var d in DriveInfo.GetDrives()) {...} Thread.Sleep(2000); }
//   работало, но грузило диск и флешка ловилась с лагом до 2 сек.
// v2: нашёл в MSDN ManagementEventWatcher - винда САМА сообщает о флешке.
//   Переделано дома под все условия: события приходят из фонового потока,
//   поэтому пробрасываем их в UI-поток через Dispatcher.BeginInvoke,
//   иначе WPF кидает InvalidOperationException (проверено на своей шкуре).
// --------------------------------------
...
namespace AvtoParkingKAI.Services
{
    ...
    public class UsbKartaService
    {
        ...
        /// Если WMI недоступен - вместо него включается резервный опрос дисков.
        /// </summary>
        public void Start()
        {
            try
        ...
        /// <summary>
        /// Ищем первую готовую съёмную флешку БЕЗ разбора роли. Null = карты нет.
        /// Терминалы этим методом больше не пользуются (при двух вставленных
        /// флешках он может вернуть не ту) - для них есть NaytiKartuKlienta /
        /// NaytiKartuDlyaVydachi / NaytiKartuDlyaKlyucha / NaytiKartuTipa ниже.
        /// </summary>
        ...
        /// Перед записью тихо стирает pass.dat, если он есть на этой же флешке -
        /// та же защита от конфликта ролей, что и в VydatNovyPropusk, только в
        /// обратную сторону. Флешка не форматируется, посторонние файлы не трогаются.
        /// </summary>
